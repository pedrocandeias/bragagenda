...
echo "Testing Gnration scraper...\n\n";

$db = new Database();

// Get Gnration source from database
$stmt = $db->getConnection()->prepare("SELECT * FROM sources WHERE scraper_class = 'GnrationScraper' LIMIT 1");
$stmt->execute();
$source = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$source) {
    echo "❌ Gnration source not found. Run setup-gnration-scraper.php first.\n";
    exit(1);
}

// Test the scraper
$scraper = new GnrationScraper($db, $source['id']);
