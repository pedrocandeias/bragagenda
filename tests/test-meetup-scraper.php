...
require_once 'scrapers/MeetupScraper.php';

echo "Testing Meetup Braga scraper...\n\n";

$db = new Database();

// Get Meetup source from database
$stmt = $db->getConnection()->prepare("SELECT * FROM sources WHERE scraper_class = 'MeetupScraper' LIMIT 1");
$stmt->execute();
$source = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$source) {
    echo "❌ Meetup source not found in database.\n";
    exit(1);
}

// Test the scraper
$scraper = new MeetupScraper($db, $source['id']);
