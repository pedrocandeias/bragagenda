...
require_once 'scrapers/EspacoVitaScraper.php';

echo "Testing Espaço Vita scraper...\n\n";

$db = new Database();

// Get Espaço Vita source from database
$stmt = $db->getConnection()->prepare("SELECT * FROM sources WHERE scraper_class = 'EspacoVitaScraper' LIMIT 1");
$stmt->execute();
$source = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$source) {
    echo "❌ Espaço Vita source not found. Run setup-espaco-vita-scraper.php first.\n";
    exit(1);
}

// Test the scraper
$scraper = new EspacoVitaScraper($db, $source['id']);
